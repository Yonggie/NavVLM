...
class InstanceMemory:
    ...
    timesteps: List[int] = []
    last_local_id_to_global_id_map: List[Dict[int, int]] = []
    ...
    def reset(self):
        ...
        self.last_local_id_to_global_id_map = [{}
    ...
    def update_instance_id(
        ...
        if self.debug_visualize:
            category_name = (
                ...
                if self.category_id_to_category_name is None else
                self.category_id_to_category_name[instance_view.category_id])
            ...
            mask = cv2.bitwise_and(mask,
                                   mask,
                                   mask=instance_view.mask.astype(np.uint8))
    ...
    def save_to_3DSG(self, env_id: int, ):
        output_list = []
        for global_inst_id, global_inst in self.instance_views[env_id].items():
            # global_inst_id: global instance id for the instance
            # inst_views: global instance with attribute -> name, category_id, instance_views
            instance_dict = {}

            instance_dict['instance_id'] = global_inst_id
            instance_dict['category_id'] = global_inst.category_id
            instance_dict['category_name'] = (
                None if self.category_id_to_category_name is None else 
                self.category_id_to_category_name[global_inst.category_id])
            pose_tmp = global_inst.instance_views[0].pose.tolist()
            # print('[instance_tracking_modules] pose_tmp:',pose_tmp)
            # pose_tmp = [ coord[0] for coord in pose_tmp ]
            pose_tmp = pose_tmp[:2]
            instance_dict['position'] = pose_tmp
            # instance_dict['project_state'] = None

            instance_dict['type'] = None
            instance_dict['location'] = None
            instance_dict['affordances'] = []
            instance_dict['state'] = None
            instance_dict['attributes'] = []
    ...
    def process_instances_for_env(
        ...
        # print("[instance tracking modules] pose shape", pose.shape)
        self.unprocessed_views[env_id] = {}  # Reset to empty dict in each step
        self.last_local_id_to_global_id_map[env_id] = self.local_id_to_global_id_map[env_id]
        ...
        # print('[instance_tracking_modules]: instance_ids', instance_ids)
        for instance_id in instance_ids:
            # skip background
            if instance_id == 0:
                continue
            # get instance mask
            instance_mask = instance_map == instance_id  # Mask -> true/false for each entry
            ...
            # print('masked shape', masked_image.shape)
            ...
            # get point cloud
            # print(instance_mask_downsampled.shape) # [128, 128]
            # print(point_cloud.shape) # [128, 128, 3]
            point_cloud_instance = point_cloud[
                instance_mask_downsampled.cpu().numpy()]
            ...
            if self.debug_visualize:
                ...
                cv2.imwrite(
                    f"{self.save_dir}/all/{self.timesteps[env_id] + 1}_{instance_id.item()}_{category_id}.png",
                    cropped_image[:, :, :],
                )
