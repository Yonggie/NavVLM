...
class GoatMatching(Matching):
    ...
    def get_matches_against_current_frame(
        ...
        **kwargs,
        ...
        for local_instance_id, inst_view in instance_memory.unprocessed_views[
            ...
            # print('[goat matching] local instances id:', local_instance_id)
        ...
        # print('[goat matching] instance ids', instance_ids)
    ...
    def get_best_match(self, scores, instance_ids, instance_map, score_thresh):
        ...
        while (
            ...
            if instance_ids is None: # No object has been detected in the current frame
    ...
    def get_goal_map_from_goal_instance(
        ...
        if goal_inst is not None and instance_goal_found is True:
            ...
            if self.goto_past_pose:
                ...
                inst_map_idx = torch.argmax(torch.sum(inst_map_idx, axis=(1, 2))) # This returns the semantic category index for the instance
                goal_map = (instance_map[inst_map_idx] == goal_inst).to(torch.float) # The binary map (H,W) with the goal info

        return goal_map, found_goal, goal_pose

    def select_and_localize_instance(
        self,
        goal_map: torch.Tensor, # (1, 1, local_h, local_w)
        found_goal: torch.Tensor, # a binary tensor (1, 1) indicating whether the goal has been found
        local_map: torch.Tensor, # (1, 36, local_h, local_w)
        lmb: torch.Tensor,  # (4) local map boundaries
        ...
        if goal_inst is None and matches is not None:
            for e in range(confidence.shape[0]): # Num of env?
                scores = confidence[e]
